...
- **`dist/` is checked into git.** After making source changes, you must run `npm run build` and commit the updated `dist/client.js` bundle. CI enforces this: the `bundle` job rebuilds and fails if the committed bundle differs. The build is byte-reproducible for a given lockfile and Node version, which is why that job pins Node 20 to match `release.yml`.
