...
const translations: Record<string, string> = {
    ...
    'A subprocess that completes as a whole, or is compensated':
        '全体として完了するか、補正されるサブプロセスです',
    'A subprocess triggered by an event in its scope':
        'スコープ内のイベントによって起動されるサブプロセスです',
    ...
    'Activities run in any order, any number of times':
        'アクティビティを任意の順序で何度でも実行します',
    'Activities run in any order; shown collapsed':
        'アクティビティを任意の順序で実行します。折りたたんで表示されます',
    'Ad-hoc': 'アドホック',
    ...
    'An unspecified step, often used as a placeholder':
        '種類を指定しないステップです。プレースホルダーとしてよく使われます',
    ...
    'Starts the process when a matching signal is broadcast':
        '一致するシグナルがブロードキャストされるとプロセスを開始します',
    ...
    'The default path, taken when no other path condition is met':
        '既定の経路です。他のどの経路の条件も満たされない場合に通ります',
    ...
    Group: 'グループ',
    Process: 'プロセス',
    'Text Annotation': 'テキスト注釈',
    'Intermediate Catch Event': '中間補足イベント',
    'Collapsed Sub Process': 'サブプロセス (折りたたみ)',
    'No Issues': '問題なし',
    'Toggle linting overlays': 'チェック表示の切り替え',
    'Issues for child elements': '子要素の問題',
    '{errors} Errors, {warnings} Warnings': 'エラー {errors} 件、警告 {warnings} 件',
};
