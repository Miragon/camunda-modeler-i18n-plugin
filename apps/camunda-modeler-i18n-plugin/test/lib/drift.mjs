...
export function analyse({ raw = false } = {}) {
    ...
    for (const [name, locale] of locales) {
        ...
        report[name] = {
            ...
            coverage: +((100 * (locale.dictionary.size - count)) / locale.dictionary.size).toFixed(
                1,
            ),
    ...
    for (const [name, stats] of Object.entries(report)) {
        ...
        if (stats.untranslated > before.untranslated) {
            regressions.push(
                `${name}: untranslated rose ${before.untranslated} -> ${stats.untranslated}`,
            );
        }
        if (stats.duplicates > before.duplicates) {
            regressions.push(
                `${name}: duplicate keys rose ${before.duplicates} -> ${stats.duplicates}`,
            );
        }
    }

    const blocking =
        missing.length + parityErrors.length + placeholderErrors.length + regressions.length;
...
export function format(result) {
    ...
    if (result.missing.length) {
        lines.push(
            '',
            `  MISSING — upstream strings with no translation (${result.missing.length}):`,
        );
    ...
    if (result.parityErrors.length) {
        ...
        result.parityErrors
            .slice(0, 20)
            .forEach((e) => lines.push(`    ${e.locale}: ${e.problem} ${JSON.stringify(e.key)}`));
    }

    if (result.placeholderErrors.length) {
        lines.push(
            '',
            `  PLACEHOLDERS — interpolation does not match the key (${result.placeholderErrors.length}):`,
        );
        result.placeholderErrors
            .slice(0, 20)
            .forEach((e) =>
                lines.push(
                    `    ${e.locale}: ${JSON.stringify(e.key)} -> ${JSON.stringify(e.value)}`,
                ),
            );
    }
    ...
    lines.push(
        '',
        result.blocking
            ? `FAILED — ${result.blocking} blocking problem(s).`
