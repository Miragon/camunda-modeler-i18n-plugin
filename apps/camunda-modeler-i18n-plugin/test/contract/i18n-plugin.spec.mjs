...
/**
 * The shared translator, imported from the same module graph the plugin uses.
 * `vi.resetModules()` in afterEach means each test re-imports a fresh instance,
 * so this must be read *after* the plugin is loaded within the same test.
 */
async function loadTranslator() {
    return (await import('../../client/bpmnjs-i18n-extension/index.js')).i18n;
}
...
describe('I18nPlugin', () => {
    ...
    describe('editor wiring', () => {
        ...
        it('adds the translate module to the BPMN editor', async () => {
            const I18nPlugin = await loadPlugin();
            const translateModule = (await import('../../client/bpmnjs-i18n-extension/index.js'))
                .default;
            const { props, subscriptions } = createProps(stored);
        ...
        it('applies the stored language to the translator', async () => {
            const I18nPlugin = await loadPlugin();
            const i18n = await loadTranslator();
            const { props } = createProps(stored);
            ...
            expect(saved).toEqual([
                {
                    key: 'i18n',
                    name: 'config',
                    value: { currentLanguage: { value: 'fr', label: 'Français' } },
                },
            ]);
        });
