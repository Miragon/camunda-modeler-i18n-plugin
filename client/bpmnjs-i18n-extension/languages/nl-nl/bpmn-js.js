...
export default {
    ...
    'A participant with its process shown inside': 'Een participant met het proces zichtbaar aan de binnenkant',
    'A subprocess that completes as a whole, or is compensated': 'Een subproces dat als geheel wordt afgerond of wordt gecompenseerd',
    'A subprocess triggered by an event in its scope': 'Een subproces dat wordt getriggerd door een event binnen zijn scope',
    'A task done by a person without any tooling or UI': 'Een taak die een persoon uitvoert zonder tooling of UI',
    ...
    'An unspecified step, often used as a placeholder': 'Een niet nader bepaalde stap, vaak gebruikt als placeholder',
    ...
    'Calls another process as a reusable subprocess': 'Roept een ander proces aan als herbruikbaar subproces',
    ...
    'Connects one element to the next in the flow': 'Verbindt het ene element met het volgende in de flow',
    ...
    'Evaluates a business rule, typically a DMN decision': 'Evalueert een business rule, meestal een DMN-decision',
    ...
    'Fires on a timer while the activity continues': 'Vuurt af op een timer terwijl de activity doorloopt',
    'Follows the path of whichever event occurs first': 'Volgt het pad van het event dat als eerste optreedt',
    'Gateways': 'Gateways',
    'Groups elements into a subprocess, shown collapsed': 'Groepeert elementen in een subproces, ingeklapt weergegeven',
    'Groups elements into a subprocess, shown inline': 'Groepeert elementen in een subproces, uitgeklapt weergegeven',
    'Handles an event that occurs while an activity is active': 'Handelt een event af dat optreedt terwijl een activity actief is',
    'Immediately ends all active paths in the scope': 'Beëindigt direct alle actieve paden binnen de scope',
    ...
    'Interrupts the activity on a defined timer': 'Onderbreekt de activity op een gedefinieerde timer',
    'Interrupts the activity on a matching signal': 'Onderbreekt de activity bij een passend signal',
    'Interrupts the activity on an escalation': 'Onderbreekt de activity bij een escalation',
    'Interrupts the activity when a condition is met': 'Onderbreekt de activity zodra een conditie is vervuld',
    'Interrupts the activity when a message is received': 'Onderbreekt de activity zodra er een message wordt ontvangen',
    'Interrupts the activity when it throws an error': 'Onderbreekt de activity zodra deze een error gooit',
    ...
    'React to something while an activity is running': 'Reageren op iets terwijl een activity loopt',
    'Reacts to a message while the activity continues': 'Reageert op een message terwijl de activity doorloopt',
    'Reacts to a signal while the activity continues': 'Reageert op een signal terwijl de activity doorloopt',
    'Reacts to an escalation while the activity continues': 'Reageert op een escalation terwijl de activity doorloopt',
    ...
    'Routes the token down one path, based on data': 'Stuurt de token op basis van gegevens naar één pad',
    ...
    'Splits the flow into parallel paths, or joins them': 'Splitst de flow in parallelle paden of voegt ze samen',
    'Start event': 'Start-Event',
    'Start events': 'Start-Events',
    'Starts an event subprocess on a message, without interrupting': 'Start een event-subproces bij een message, zonder te onderbreken',
    'Starts an event subprocess on a signal, without interrupting': 'Start een event-subproces bij een signal, zonder te onderbreken',
    'Starts an event subprocess on a timer, without interrupting': 'Start een event-subproces op een timer, zonder te onderbreken',
    'Starts an event subprocess on an escalation': 'Start een event-subproces bij een escalation',
    'Starts an event subprocess on an escalation, without interrupting': 'Start een event-subproces bij een escalation, zonder te onderbreken',
    'Starts an event subprocess to handle compensation': 'Start een event-subproces om compensatie af te handelen',
    'Starts an event subprocess when a condition is met, without interrupting': 'Start een event-subproces zodra een conditie is vervuld, zonder te onderbreken',
    'Starts an event subprocess when an error is thrown': 'Start een event-subproces zodra er een error wordt gegooid',
    ...
    'Starts the process when a matching signal is broadcast': 'Start het proces zodra een passend signal wordt uitgezonden',
    'Starts the process when a message is received': 'Start het proces zodra er een message wordt ontvangen',
    ...
    'Takes every outgoing path whose condition is met': 'Neemt elk uitgaand pad waarvan de conditie is vervuld',
    'Tasks': 'Tasks',
    'Terminate end event': 'Terminate-End-Event',
    'The default path, taken when no other path condition is met': 'Het standaardpad, dat wordt genomen als geen enkele andere padconditie is vervuld',
    ...
    'Triggers compensation of completed activities': 'Triggert de compensatie van afgeronde activities',
    'User task': 'User-Task',
    'Wait for something to happen before continuing': 'Wachten tot er iets gebeurt voordat ze verdergaan',
    'Waits for a defined time or duration': 'Wacht op een gedefinieerd tijdstip of een gedefinieerde duur',
    'Waits for a matching signal to be broadcast': 'Wacht tot er een passend signal wordt uitgezonden',
