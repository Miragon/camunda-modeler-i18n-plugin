...
async function resolveModelerTag(spec) {
    ...
    const release = await get('https://api.github.com/repos/camunda/camunda-modeler/releases/latest', true);
...
async function modelerDependencies(tag) {
    const manifest = JSON.parse(await get(
        `https://raw.githubusercontent.com/camunda/camunda-modeler/${tag}/client/package.json`
    ));

    const deps = { ...manifest.dependencies, ...manifest.devDependencies };
    const interesting = [
        'bpmn-js', 'dmn-js', 'camunda-bpmn-js', 'camunda-dmn-js',
        'bpmn-js-properties-panel', 'dmn-js-properties-panel',
        '@bpmn-io/properties-panel', 'bpmn-js-element-templates'
    ];
...
if (writeDeps) {
    ...
    manifest.supportedModeler = tag.replace(/^v/, '');
    ...
    for (const [ name, range ] of Object.entries(deps)) {
        ...
        if (DIRECT_DEPS.includes(name)) {
            manifest.devDependencies[name] = exact;
        } else {
            overrides[name] = exact;
        }
    }

    manifest.overrides = Object.fromEntries(Object.entries(overrides).sort());
    fs.writeFileSync(path.join(ROOT, 'package.json'), JSON.stringify(manifest, null, 4) + '\n');

    console.log('\nPinned package.json to this release. Now run:');
    console.log('  npm install && npm run harvest && npm run check:translations');
}

const bpmnJsTag = `v${clean(deps['bpmn-js'])}`;
const keys = JSON.parse(await get(
