...
export const CACHE = process.env.MODELER_CACHE
    ?? path.join(os.homedir(), '.cache', 'camunda-modeler-e2e');
...
export function createProfile({ language, source = ROOT } = {}) {
    ...
    const config = {
        ...
        'editor.privacyPreferences': { ENABLE_CRASH_REPORTS: false, ENABLE_USAGE_STATISTICS: false },
    ...
    if (language) {
        ...
        config.plugins = { i18n: { config: { currentLanguage: { value: language, label: language } } } };
    }
